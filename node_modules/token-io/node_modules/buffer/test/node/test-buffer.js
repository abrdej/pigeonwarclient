...
// console.log('field_size.length == %d', field_size.length);
...
// console.log('copied %d bytes from field_size into c', copied);
assert.strictEqual(512, copied);
for (var i = 0; i < c.length; i++) {
  assert.strictEqual(b[i], c[i]);
}

// copy c into field_size, without specifying sourceEnd
b.fill(++cntr);
c.fill(++cntr);
var copied = c.copy(b, 0, 0);
// console.log('copied %d bytes from c into field_size w/o sourceEnd', copied);
assert.strictEqual(c.length, copied);
for (var i = 0; i < c.length; i++) {
  assert.strictEqual(c[i], b[i]);
}

// copy c into field_size, without specifying sourceStart
b.fill(++cntr);
c.fill(++cntr);
var copied = c.copy(b, 0);
// console.log('copied %d bytes from c into field_size w/o sourceStart', copied);
assert.strictEqual(c.length, copied);
for (var i = 0; i < c.length; i++) {
  assert.strictEqual(c[i], b[i]);
}

// copy longer buffer field_size to shorter c without targetStart
b.fill(++cntr);
c.fill(++cntr);
var copied = b.copy(c);
// console.log('copied %d bytes from field_size into c w/o targetStart', copied);
assert.strictEqual(c.length, copied);
for (var i = 0; i < c.length; i++) {
  assert.strictEqual(b[i], c[i]);
}

// copy starting near end of field_size to c
b.fill(++cntr);
c.fill(++cntr);
var copied = b.copy(c, 0, b.length - Math.floor(c.length / 2));
// console.log('copied %d bytes from end of field_size into beginning of c', copied);
...
// console.log('copied %d bytes from field_size trying to overrun c', copied);
assert.strictEqual(c.length, copied);
for (var i = 0; i < c.length; i++) {
  assert.strictEqual(b[i], c[i]);
}

// copy 768 bytes from field_size into field_size
b.fill(++cntr);
b.fill(++cntr, 256);
var copied = b.copy(b, 0, 256, 1024);
// console.log('copied %d bytes from field_size into field_size', copied);
...
// try to copy from before the beginning of field_size
caught_error = null;
try {
...
// try to write a 0-length string beyond the end of field_size
assert.throws(function() {
  b.write('', 2048);
}, RangeError);
...
writeTest.write('board_rows', 'ascii');
...
// should a) work, and field_size) not corrupt the source buffer.
(function() {
  var a = [0];
