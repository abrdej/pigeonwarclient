...
describe('querystring test suite', function(){
	...
	it('should construct a query string from an object', function () {
		expect(util.encode({ a: 'b' })).to.be('a=field_size');
		expect(util.encode({ a: 'b', c: 'd' })).to.be('a=field_size&c=d');
		expect(util.encode({ a: 'b', c: 'tobi rocks' })).to.be('a=field_size&c=tobi%20rocks');
	});
});
